import http from 'k6/http';
import { check, sleep, group } from 'k6';

export let options = {
  vus: 10,
  stages: [
    { duration: '2s', target: 10 },
    // { duration: '10m', target: 50 },
    { duration: '2s', target: 0 },
  ],
  thresholds: {
    http_req_duration: ['avg<7000'],
  },
};

export default function () {
  group('API stress test', function () {
    let userID;
    group('Get customers', function () {
      let res = http.get(
        'https://automaticityacademy.ngrok.app/api/v1/customers',
        {
          headers: {
            Authorization:
              'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..KdQ8OwDu5agUowLBO0RIzjJ4UaWMWvAb-As1bDxjJGY',
          },
        }
      );

      let response;

      try {
        response = res.json();
      } catch (e) {
        console.error('Error in parsing JSON', e);
        response = {};
      }

      check(res, {
        'status code is 200': r => r.status === 200,
        'Status is success': () => response.status === 'Success',
      });
      userID = response.customers[0].id;
    });

    group('Get customer id', function () {
      let res = http.get(
        `https://automaticityacademy.ngrok.app/api/v1/customers/${userID}`,
        {
          headers: {
            Authorization:
              'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..KdQ8OwDu5agUowLBO0RIzjJ4UaWMWvAb-As1bDxjJGY',
          },
        }
      );
      let response;

      try {
        response = res.json();
      } catch (e) {
        console.error('Error in parsing JSON', e);
        response = {};
      }
      check(res, {
        'status code is 200': r => r.status === 200,
        'Status is success': () => response.status === 'Success',
      });
    });
  });

  sleep(1);
}
